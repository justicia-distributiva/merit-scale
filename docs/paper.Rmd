...
) %>% kable_styling(full_width = table_format2, font_size = 9,position = "center");sumdat04
```
...
tb.foot <- paste0("Standardised factor loadings using DWLS estimator")
...
tb.fm01<- kable(tb.fm, format = table_format, booktabs = T,linesep = "", escape = FALSE,align = "lcccccccccccc",col.names = tb.col, caption = loadfit) %>% 
          ...
          add_indent(c(9:12))  %>%
          footnote(general = tb.foot, number = c("CFI = Comparative fit index (scaled); ", " RMSEA = Root mean square error of approximation (scaled); "));tb.fm01
