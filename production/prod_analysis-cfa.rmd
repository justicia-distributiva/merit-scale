...
fm.df <- data.frame(t(fm.df)); colnames(fm.df) <- c("M1.1","M2.1","M3.1")
#------ merge ------#
...
dat_per <- dat04 %>% select("Effort"=perc_effort,"Talent"=perc_talent, "Rich Parents"=perc_wpart, "Contacts"=perc_netw)    
dat_pref <- dat04 %>% select("Effort"=pref_effort,"Talent"=pref_talent,"Rich Parents"=pref_wpart, "Contacts"=pref_netw)   

tmp_per<- melt(dat_per)
tmp_per$tipo<- "Perceptions"

tmp_prf<- melt(dat_pref)
tmp_prf$tipo<- "Preferences"

tmp <-rbind(tmp_per,tmp_prf) 

save(tmp, file = "output/images/grafico_descriptivo.RData")
