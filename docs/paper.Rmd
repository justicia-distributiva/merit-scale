...
pacman::p_load(knitr,kableExtra, dplyr, captioner)
```
...
## The black-box of meritocratic beliefs
...
```{r echo=FALSE}
 ...
 kable(tab01,table_format,booktabs = T, linesep = "",col.names = cnames, caption = tb) %>%
   kable_styling(full_width = T,latex_options = c("hold_position","scale_down"),position = "center",font_size = 14, bootstrap_options=c("striped", "bordered")) %>%
...
![`r diam1`](../input/images/app_mod.png){ width=80% }
...
![`r diam2`](../output/images/meas01.png)
...
The study was pre-registered ... (link) ***
...
sumdat04<- kable(table04b,format =table_format, booktabs= T, escape = FALSE,align = "l",linesep = "", caption = tades
) %>% kable_styling(full_width = table_format2, font_size = 14,position = "center");sumdat04
```

El gráfico XX muestra ...
A partir del gráfico se observa que en general hay una mayor percepción de elementos no meritocráticos que meritocráticos, mientras que en el caso de preferencias ocurre lo opuesto. En cuanto a las preferencias, llama la atención el rol preponderante del esfuerzo por sobre el talento.
...
![`r liker`](../output/images/plotlikert.png){ width=60% }

</center>

El Grafico XX muestra ....
...
```{r include=FALSE}
cap <- "polychoric correlation plot"
if (knitr::is_html_output()) {fb <- fig_nums("fb",cap)} else {fb <- cap}
```

<center>

![`r fb`](../output/images/corpoly.png){ width=60% }
...
Los modelos confirmatorios fueron estimados en primer lugar para cada uno de los tres administration-sets. Los resultados se presentan en la Tabla XX
...
tb.fm01<- kable(tb.fm, format = table_format, booktabs = T,linesep = "", escape = FALSE,align = "lcccccccccccc",col.names = tb.col, caption = loadfit) %>%
