...
fm01<- data.frame(t(data.frame(fitmeasures(fs1_o, output ="matrix")[c("chisq","df","cfi","tli","rmsea"),]))); row.names(fm01) ="M1.1"

#------chi2, df------#
fm04.1 <- fm01 %>% select(chisq,df) 
...
#------CFI, RMSEA------#
fm04.2 <- fm01 %>% select(cfi,tli,rmsea)
...
tb.fm2<- tb.fm2 %>% 
  mutate(vars=c(labs,"$\\chi^2\\text{(df)}$","$\\text{CFI}$","$\\text{TLI}$","$\\text{RMSEA}$","$N$")) %>%
