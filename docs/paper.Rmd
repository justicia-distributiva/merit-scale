...
We performed two further analyses in order to add evidence regarding the reliability and validity of the scale. Firstly we test the stability of the scale scores along time through longitudinal measurement invariance methods, and secondly we analyze the convergent validity by assessing the association of the merit scale with related concepts and correspondent measurements.

### Measurement invariance

*Datos*

Para el desarrollo de esta sección, se empleó una fuente de datos externa a los resultados presentados en el apartado anterior. Los datos provienen de un estudio realizado durante el primer semestre de 2020, dentro del cual se realizó una medición usando la escala a través de aleatorización completa de los indicadores, lo cual corresponde a la modalidad de aplicación del Grupo 3 presente en la Figura \@ref(fig:appmod). Después de la eliminación de casos perdidos por lista, obtuvimos una muestra analítica final de 1242 casos donde 605 (48,71%) son mujeres y 637 (51,28%) son hombres. El 48,84% de ellos tienen 45 años o más, y el 33,97% poseen educación terciaria o superior. En la Tabla \@ref(tab:fcloads-inv) se pueden observar los resultados del ajuste de la escala en esta muestra. Análisis El modelamiento para el testeo invarianza consiste en una serie de modelos anidados a los que progresivamente se le van incorporando restricciones sobre los parámetros del modelo de medición , permitiendo esta progresión localizar la fuente de una violación de invarianza, lo cual ha sido tomando mayor relevancia en los estudios sociológicos a través de encuestas, principalmente por la validez del uso de escalas de medición en estudios de carácter comparado en diversos contextos sociales y culturales [@Davidovetal2014]. En esta sección emplearemos un análisis de invarianza entre grupos, donde se espera que la estructura factorial sea independiente de la pertenencia a las la muestra correspondiente al estudio original y a los datos del estudio más reciente. Para el desarrollo del test de invarianza, los autores sostienen que el modelamiento debe realizarse a través de cuatro niveles o tipos de restricción progresiva [@Milfont2010; @millsap_Statistical_2011; @vandeschoot_checklist_2012]:

1.  **Configural:** Se estima el Modelo solo de indicando la estructura  de este, razón por lo cual todos los parámetros se estiman libremente.

2.  **Débil:** Posteriormente, en el modelo se restringen las cargas factoriales de cada ítem, haciéndolas idénticas en ambas mediciones.

3.  **Fuerte:** Luego, se estima el modelo restringiendo las cargas factoriales y los interceptos de cada ítem, haciéndolos idénticos para cada ambas mediciones.

4.  **Estricta:** Finalmente, se estima un modelo donde se restringen las cargas factoriales, los interceptos y las varianzas de error para cada ítem, haciéndolos idénticos para ambas mediciones.
