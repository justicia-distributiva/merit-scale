...
```{r include=FALSE}
cap <- "Dimensions of meritocratic beliefs."
if (knitr::is_html_output()) {ta <- table_nums("ta",cap)} else {ta <- cap}
```
...
```{r echo=FALSE}
 ...
 kable(tab01,table_format,booktabs = T, linesep = "",col.names = cnames, caption = tb) %>%
   kable_styling(full_width = T,latex_options = c("hold_position","scale_down"),position = "center",font_size = 12) %>%
...
![`r diam1`](../input/images/app_mod.png){ width=90% }
...
Como puede observarse en la tabla 3, los indicadores poseen valores que van desde el 1 al 5. Se observan promedios desde 2,41 correspondiente a la preferencia de distribucion según contactos, hasta 3,89, correspondiente a la preferencia de distribucion según esfuerzo. Ambos indicadores son coherentes con la adhesión  a las creencias meritocraticas, privilegiando la distribucion meritocratica por sobre aquella que se debe a ventajas externas. 
...
A partir del grafico liker  se observa que, en general, hay una mayor percepción de aspectos no meritocráticos que meritocráticos, mientras que en el caso de preferencias ocurre lo opuesto. En cuanto a las preferencias, llama la atención el rol preponderante del esfuerzo por sobre el talento. 
...
![`r liker`](../output/images/plotlikert.png){ width=70% }

</center>

\pagebreak

El grafico de correlaciones policoricas evidencia relaciones de moderada a alta intensidad entre los indicadores que corresponden al mismo factor, mientras que se observan correlaciones de baja intensidad en el resto de los cruces. Destacan además las relaciones entre las percepciones y preferencias de tipo meritocraticas, lo que no ocurre con los indicadores no meritocraticos. 
...
![`r fb`](../output/images/corpoly.png){ width=70% }
...
En suma, los análisis descriptivos señalan una relativa adhesión a la moral meritocratica, las cuales, a modo general, se expresan en una mayor preferencia de distribucion según criterios meritocraticos y una menor preferencia de criterios no meritocraticos. Igualmente, es observable una relativamente baja percepcion de distribucion meritocratica. Además se observa una relación coherente entre los indicadores según lo propuesto en el modelo teórico en el preregistro. 
