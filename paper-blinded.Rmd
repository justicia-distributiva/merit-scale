...
- _Brief_, as to be used in regular public opinion surveys. To this regard, it differs from the proposal of  "Preference for the Merit Principle Scale" [@davey_preference_1999], as they use 15 items for just one dimension (aside from the problem of reverse-coding non-meritocratic items).
...
Table \@ref(tab:sum-fit-issp) shows the results of the estimation of two confirmatory models. The first one (First Order) corresponds to a model that estimate four factors, each for one of the dimensions presented in \@ref(tab:table-issp), showing only regular fit indicators (CFI=0.959, TLI=0.944, RMSEA=0.098, $\chi^2$(df=48)= 21308.535). Further analysis showed that the sources for poor fit were mostly related to the items of the non-meritocratic perceptions, which as mentioned above displayed correlations among them not taken into account in this model specification.
...
Although it was possible to get an adequate fit of the model to the pooled data from ISSP, there are several aspects to take into account when using these items for measuring meritocracy. First of all, the items were not designed based on this conceptual approach and a first estimation approach (first order factor model) reveals some problems of fit. Secondly, some of the items in similar dimensions refer to different concepts, as for instance meritocratic perceptions are related to ambition and hard work, whereas meritocratic preferences deal with how well a job is done. A better operationalization would require more alignment between the different sides of the same concept. And thirdly, the scale is rather unbalanced given he larger amount of non-meritocratic perception items, producing correlated errors. All these limitations are considered in the design of Study 2.

With regard to the meritocratic / non-meritocratic distinction in previous studies, we can preliminary conclude that the wide use of reverse-coding for non-meritocratic items attempting to measure meritocracy is not an adequate operationalization of the construct. Meritocracy and non-meritocracy are related but different constructs, meaning that it is theoretically possible to find high and lower level of both. This finding opens a series of avenues for future research that are developed in the conclusions.
...
The proposed scale of perceptions and preferences relating to meritocracy consisted of eight indicators, two for each of the four dimensions listed earlier: Perceptions (meritocratic/non-meritocratic) and preferences (meritocratic/non-meritocratic). This design is oriented by two main criteria. The first one is usability in survey research, for which the length of the scale is restricted due to space reasons in such studies. Nevertheless, at the same time the scale should offer possibilities of accounting for measurement error through factor analysis techniques. Although a rule of thumb in latent  measurement suggest at least three indicators per dimension, we still opted for two in order to count with a brief scale. Although such decision posits some limitation for modeling measurement error, still allows for factor analysis and is a common approach in previous social survey studies with limited items (for instance see @davidov_measurement_2009).
...
Based on the measurement invariance analysis for two datasets, we found evidence supporting the equivalence of the scale when applied in different samples. This gives a more robust ground when using this measure for comparing groups, as for instance country comparison. Although the results ar promising, still the analysis was performed in two samples in one country (Chile) and it requires further examination with social surveys in other societies.
