...
![Tabla 2: Dimensiones](../input/images/tabla2.png)

6 XX ojo con esta imagen, titularla en el documento, no en la imagen misma
...
7 XX agregar número de sujetos por grupo

8 XX referenciar y titular figuras y tablas compatible con html/pdf

![Diagrama 1: Study design](../input/images/app_mod.png)
...
1 XX producir tabla y llamar directamente desde la fuente o como imagen
...
```{r echo=FALSE}
library(ggplot2)
load(file = "output/images/grafico_descriptivo.RData")
ggplot(tmp, aes(x = variable, y = value)) + geom_boxplot() +  facet_wrap(~ tipo)
```
...
3 - XX Sobre tablas en general, entonces:
...
4 XX En tabla de cargas factoriales, los fit aparecen corridos hacia la izquierda
...
5 XX explicar mejor lo que viene: invarianza de qué y por qué se realiza.
