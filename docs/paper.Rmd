...
knitr::opts_chunk$set(warning = FALSE,message = FALSE, cache = TRUE,out.width = '75%',fig.pos= "H")
...
El Grafico \@ref(fig:corpoly) muestra ....
...
sumtable02<- kable(sum_fit,digits = 3,format = "html",row.names = F,booktabs=T, caption = "Summary fit indices according group",col.names = colnames,escape = FALSE) %>%
...
```{r include=FALSE}
cap <- "Items according to dimension."
# if (knitr::is_html_output()) {te <- table_nums("te",cap)} else {te <- cap}
```
...
```{r include=FALSE}
cap <- "Representativeness of the sample."
# if (knitr::is_html_output()) {aax <- table_nums("aax",cap)} else {aax <- cap}
```
